...
class Piper(Robot):
    ...
    def connect(self, calibrate: bool = True) -> None:
        # 1. Connect CAN bus first
        self.sdk.connect()
        # 2. Connect cameras (this can take 10-20 seconds)
        for cam in self.cameras.values():
            cam.connect()
        # 3. Enable arm LAST - right before teleop loop starts
        self.sdk.enable()
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        # action keys are joint_0.pos through joint_6.pos with raw millidegree values
        positions = [action.get(f"joint_{i}.pos", 0) for i in range(7)]
